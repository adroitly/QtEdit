		...
		for (k = 0; k < 5; k ++)
		{
			...
			for (j = 0; j < _tempVector->Sprites.size(); j ++)
			{
				...
				if (k == 1 && _tempSpritePoint->Is_BodyFra)
				{
					...
					_tempDrawNode->setRelativeVertices(_tempDrawNode->Vertices, Vec2(_tempSpritePoint->_width, _tempSpritePoint->_height));
				}
				...
				if (k == 0)
				{
					x = _tempSpritePoint->_width - _IN_Width;
					y = _tempSpritePoint->_height - _IN_Height;
				...
				else
				{
					...
					{
						x = (_tempDrawNode->Relativevertices[0].x + _tempDrawNode->Relativevertices[2].x) / 2.0 - _tempSpritePoint->_width;
						y = (_tempDrawNode->Relativevertices[0].y + _tempDrawNode->Relativevertices[2].y) / 2.0 - _tempSpritePoint->_height;
						enabled = true;
					}
					else
					{
